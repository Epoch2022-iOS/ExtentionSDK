...
extension BKCardView {
    ...
    private func addTimer() {
        timer = BKGCDTimer(interval: scrollTimeInterval, delaySecs: scrollTimeInterval, action: { [weak self] _ in
            self?.nextPage()
        })
        timer?.start()
    }
    ...
    private func scrollToCenterAnimated(animated: Bool) {
        ...
        _collectionView.safeScrollToItem(at: IndexPath(item: selectedIndex, section: 0), at: .centeredHorizontally, animated: animated)
    }
