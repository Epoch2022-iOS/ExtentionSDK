...
extension String {
    ...
    /// 无法保存图片
    static let canNotSavePhoto = "无法保存图片"
    /// 分享图片生成
    static let shareImageGeneration = "分享图片生成"
    /// 找不到轨迹
    static let canNotFindTrack = "找不到轨迹"
    /// 手机号码格式错误
    static let mobileFormatError = "手机号码格式错误"
    /// 邮箱格式错误
    static let emailFormatError = "邮箱格式错误"
    /// 身份证号校验错误
    ...
    /// 搜索关键字不能为空
    static let keywordNotEmpty = "搜索关键字不能为空"
}
...
extension String {
    ...
    var formatTime: String {
        ...
        if self.contains("."), self.components(separatedBy: ".").count-1 == 2 {
            let parts = self.components(separatedBy: ".")
            let year = parts[0]
            let month = parts[1]
            let day = parts[2]
            newTime = "\(year).\(Int(month)!).\(Int(day)!)"
        }
        return newTime
    }
...
extension String {
    ...
    func isValidUserName(from: Int = 2, to: Int = 11) -> Bool {
        ...
        return userName.count >= from && userName.count <= to
...
//        return userName =~ regex
    ...
    func bk_addAsterisk(loc: Int = 3, len: Int = 4) -> String {
        ...
        return self.replacingOccurrences(of: self, with: asterisk, range: self.toRange(from: NSMakeRange(loc, len)))
    }
