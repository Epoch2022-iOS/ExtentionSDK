...
extension UIScrollView: UIGestureRecognizerDelegate {
    ...
    func bk_scrollBottom(animated flag: Bool = true) {
        let offset: CGFloat = self.contentSize.height - self.bounds.size.height
        if offset > 0 {
            self.setContentOffset(CGPoint(x: 0, y: offset), animated: flag)
        }
    }
...
extension UIScrollView {
    ...
    public func bk_contentCapture(_ completionHandler: @escaping (_ capturedImage: UIImage?) -> Void) {
        ...
        self.swRenderImageView { [weak self] (capturedImage) in
...
//        let swizzledMethod: Method = class_getInstanceMethod(object_getClass(self), Selector(("bk_setNewFrame:")))!
