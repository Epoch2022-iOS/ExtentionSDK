...
extension Date {
    ...
    enum StatRankType: Int {
        case day = 1
        case week
        case month
        
        var style: (des: String, clubIcon: String) {
            switch self {
            case .day: return ("日", "icon_club_rankshare_day")
            case .week: return ("周", "icon_club_rankshare_week")
            case .month: return ("月", "icon_club_rankshare_month")
            }
        }
    }
    ...
    static func secondsToTimeStringPart(seconds: Int) -> (d: String, h: String, m: String, s: String) {
        // 天数计算
        ...
        // 秒计算
        let s = (seconds)%60
        return (String(format: "%02lu", d), String(format: "%02lu", h), String(format: "%02lu", m), String(format: "%02lu", s))
    }
