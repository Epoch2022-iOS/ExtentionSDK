...
class BKCircleGradientProgressView: UIView {
    ...
    struct Style {
        ...
        /// 进度条颜色
        var progressColors = [BKColorDef.green94]
        /// 进度槽颜色
        var trackColor = BKColorDef.gray248
        /// 是否展示头部圆点图标
    ...
    /** 进度槽颜色*/
    var trackColor: UIColor = BKColorDef.gray248 {
        didSet {
            kStyle.trackColor = trackColor
        }
    }
    
    /** 进度条完成部分的渐变颜色，设置单个为纯色，设置多个为渐变色*/
    var progressColors: [UIColor] = [BKColorDef.green94] {
        didSet {
            kStyle.progressColors = progressColors
        }
    }
