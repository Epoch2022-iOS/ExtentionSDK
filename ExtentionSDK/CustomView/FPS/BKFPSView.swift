...
// MARK: - FPS展示视图
public class BKFPSView: NSObject {
    ...
    /// 计时器
    private let handler = BKFPSHandler.shared
    
    /// 单例
    public  static let shared = BKFPSView()
    
    private override init() {
        super.init()
        if fpsLabel == nil {
            fpsLabel = UILabel(frame: CGRect(x: 0, y: 0, width: 100, height: 30))
        }
        ...
        if window == nil {
            window = UIWindow(frame: CGRect(x: 0, y: 0, width: 100, height: 30))
        }
    ...
    @objc private func handlePanGes(pan: UIPanGestureRecognizer) {
        ...
        let translationX = (window?.center.x ?? 0) + translation.x
        let translationY = (window?.center.y ?? 0) + translation.y
        window?.center = CGPoint(x: translationX, y: translationY)
...
class BKFPSHandler: NSObject {
    ...
    /// 单例
    fileprivate static let shared = BKFPSHandler()
    private override init() {
        super.init()
        if displayLink == nil {
    ...
    /// 暂停监测
    open func pauseMonitoring() {
        displayLink?.isPaused = true
    }
    
    /// 移除监测
    open func removeMonitoring() {
        pauseMonitoring()
